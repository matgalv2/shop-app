...
class Validator[A](val func: A => Boolean, val description: Validator.FailureDescription)
  extends ValidationRunner[ValidationRunner.Id, A] with ApplySyntax with FunctorSyntax{

  override def validator: Validator[A] = this

  private val chain:  (A => Boolean, A => Boolean)  => ((Boolean, Boolean) => Boolean) => A => Boolean = (fun1, fun2) => operator => x => {
  ...
  def optional(option: Option[A]): Validation[Validator.FailureDescription, Option[A]] = ValidationRunner.ForOption(this).run(option)
...
object Validator {
  ...
  val capitalized: Validator[String] = new Validator( string => if(string.isEmpty) false else string.head.isUpper, s"capitalized ")

  val contains: String => Validator[String] = substring => new Validator(string => string.contains(substring), s"contains: $substring")

  val uuid: Validator[String] = new Validator(string => Try(UUID.fromString(string)).isSuccess, s"incorrect uuid format")
  ...
  def inRange(range: Range): Validator[Int] = new Validator(
    value => range.contains(value),
    s"value needs to be in range $range"
  )
  ...
  def validOrCheck[A, B <: NotValidated[A]](option: Option[B]): Validation[FailureDescription, Option[A]] = option match {
